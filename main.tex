...
\textbf{Validation} is purely simulation-based. Ground truth is obtained from
...
\textbf{Verification} of the fusion logic, in particular the obstacle-ID
propagation of Section~\ref{sec:objid}, is provided by the standalone unit test.
It mocks \texttt{cv2}, \texttt{carla}, and
\texttt{sklearn} and exercises nine cases of obstacle-ID propagation through
\texttt{LidarFusion} (Table~\ref{tab:testcases}).
...
\subsection{What is Novel About This Integration}

Compared with a textbook LiDAR integration, several aspects of this project's
approach are distinctive.

\begin{itemize}
    \item \textbf{Conservative dual-sensor consensus, not replacement.} A common
    pattern is to replace a weaker sensor with LiDAR. Here the camera detector
    is deliberately retained and the fused action is the \emph{worst case} of
    the two (\texttt{lidar\_fusion.py}). LiDAR \emph{augments} rather than
    supplants the camera.

    \item \textbf{Two-strategy matching with a safe fallback.} Fusion first
    tries precise bounding-box projection, then a looser angle gate, then
    accepts a camera-only estimate. The explicit ``safe-by-default'' policy ---
    documented in \texttt{docs/SENSOR\_COMPARISON.md} --- keeps the vehicle from
    going blind when LiDAR fails to match, while still tagging every log row
    with its true detection source so the bias can be filtered out at analysis
    time.

    \item \textbf{Asymmetric temporal filtering.} The obstacle detector treats
    danger escalation and de-escalation asymmetrically: getting closer is
    believed instantly, getting farther is damped. This safety-biased EMA and
    the matching action hysteresis are an uncommon, deliberately conservative
    design.

    \item \textbf{Built-in, honest evaluation.} The integration ships with its
    own per-frame and per-obstacle metrics logger and an accompanying document
    that explicitly warns which log is biased and why. The system measures and
    discloses its own weaknesses rather than only reporting favourable numbers.
\end{itemize}

\subsection{Summary of Key Results and Observations}
...
\begin{itemize}
    ...
    \item \textbf{Coverage.} LiDAR detection coverage is limited: in a
    representative per-obstacle log only $5.7\,\%$ of rows were confirmed by both
    sensors, because LiDAR returns are often too sparse at range to clear the
    fusion gates.
\end{itemize}

\subsection{Insights and Engineering Decisions}
...
Mean relative error & Camera $6.14\,\%$ vs.\ LiDAR $5.20\,\%$ --- both within an acceptable range for mid-distance detection. & Camera is viable as a redundant / backup sensor. \\
...
\subsection{Were the Objectives Achieved?}

Cross-referencing the three objectives from Section~\ref{sec:methodology}:

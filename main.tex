...
\paragraph{Progress in Perception-Aware Decision Making}

Another important progress point is adjacent lane detection. The lane detector now estimates the ego lane and also identifies whether the left or right adjacent lane is clear enough for overtaking. This lane layout information is used by the decision-making layer before initiating a lane change. If the vision-based lane layout is uncertain, the agent can fall back to CARLA map topology, which improves robustness when lane markings are weak or partially lost.

\paragraph{Progress in Safety Handling}

Safety handling was also strengthened. Lane changes are only allowed when the ego speed, lead-vehicle distance, and speed difference satisfy the overtake conditions, and a cooldown timer prevents repeated maneuvers. If lane detection confidence drops or the lane is lost, the vehicle transitions to a safer stop-and-hold behavior instead of continuing aggressive control.

\paragraph{Implementation Summary}

Overall, the progress achieved in this module is the development of a perception-driven trajectory planning pipeline that integrates lane keeping, ACC-based longitudinal control, adjacent lane detection, and MPC-style lane-change steering. This gives the system the ability to perform smoother and safer lane changes while responding conservatively to uncertain perception or unsafe traffic conditions.
